...
const communicationManager = {
    send: (data: any) => {
        console.log(`[DesignImage] ${data.type}:`, data);
    }
};
    ...
    try {
        ...
        if (source_images && source_images.length > 0) {
            ...
            processedImages = await Promise.all(
                imageArray.map(async (image): Promise<string> => {
                    ...
                    } else if (!image.startsWith('http://') && !image.startsWith('https://') && !image.startsWith('data:image/')) {
                        // It's likely a local file path (absolute or relative), load and convert to base64
                        try {
                            console.log(`[ImageAgent] Processing local image: ${image}`);
                            
                            // Resolve to absolute path if relative
                            const absolutePath = path.isAbsolute(image) ? image : path.resolve(image);
                            console.log(`[ImageAgent] Resolved to absolute path: ${absolutePath}`);
                            ...
                            throw new Error(`Failed to process local image: ${image}`);
                        }
                    } else if (image.startsWith('data:image/')) {
                        // It's already a base64 image, just resize if needed
                        return await resizeImageIfNeeded(image);
                    } else {
                        // This should not happen given our conditions above
                        throw new Error(`Unexpected image format: ${image.substring(0, 50)}`);
                    }
                })
            );
        }
        ...
        for (let i = 0; i < imageDataUrls.length; i++) {
            ...
            if (!output_path) {
                ...
                const fileName =
                    number_of_images === 1
                        ...
                        : `${prefix}_${timestamp}_${i + 1}.png`;
                targetPath = path.join(
                    getOutputDir(),
                    'generate_image',
                    fileName
                );
            } else if (!path.isAbsolute(output_path)) {
                // output_path is a subdirectory name, create it under getOutputDir()
                const baseDir = getOutputDir();
                const outputDir = path.join(baseDir, output_path);
                console.log(`[ImageAgent] Creating images in: ${outputDir} (base: ${baseDir}, subdir: ${output_path})`);
            ...
            } else if (fs.existsSync(output_path) && fs.statSync(output_path).isDirectory()) {
                // output_path is an absolute directory, create filename inside it
                const fileName =
                    number_of_images === 1
                        ? `${prefix}_${timestamp}.png`
                        : `${prefix}_${timestamp}_${i + 1}.png`;
                targetPath = path.join(output_path, fileName);
                ...
                if (pathParts.length > 1) {
                    ...
                    targetPath = output_path + `_${i + 1}.png`;
                }
            } else if (output_path && !output_path.endsWith('.png')) {
                // If output_path is a directory or doesn't have extension, add .png
                targetPath = output_path + '.png';
            } else if (output_path) {
                // output_path provided as-is
                targetPath = output_path;
            } else {
                // Default case - should not happen but handle it
                const fileName =
                    number_of_images === 1
                        ? `${prefix}_${timestamp}.png`
                        : `${prefix}_${timestamp}_${i + 1}.png`;
