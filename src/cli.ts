...
program
    .command('agent')
    .description('Run the design agent with a prompt')
    .argument('<prompt>', 'The design prompt for the agent')
    .option('-v, --verbose', 'Enable verbose logging')
    .action(async (prompt, options) => {
        try {
            ...
            console.error('❌ Error running agent:', error instanceof Error ? error.message : String(error));
            if (options.verbose) {
                console.error(error instanceof Error ? error.stack : error);
            }
            process.exit(1);
